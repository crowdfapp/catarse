...
CREATE OR REPLACE VIEW "1"."project_details" AS 
 SELECT p.id AS project_id,
    ...
    (case 
        when "current_user"() = 'admins'::name then nullif(btrim(array_agg(distinct admin_tags_lateral.tag_list)::text, '{}'), 'NULL')
        else null::text end
    ) as admin_tag_list,
    nullif(btrim(array_agg(distinct tags_lateral.tag_list)::text, '{}'), 'NULL') as tag_list,
    ct.id as city_id,
    (case when "current_user"() = 'admins'::name then admin_notes else null::text end) as admin_notes,
    (case when "current_user"() = 'admins'::name then service_fee else null::numeric end) as service_fee
   FROM projects p
     ...
     left join lateral (
        ...
        from taggings tgs
            ...
            where tgs.project_id = p.id and tgs.public_tag_id is not null
